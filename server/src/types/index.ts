...
export interface DetectionResult {
  id: string;
  caseId: string;
  faceForgeryScore: number;       // 0 - 100
  temporalScore: number;          // 0 - 100
  audioVisualScore: number;       // 0 - 100
  metadataScore: number;          // 0 - 100
  provenanceStatus: ProvenanceStatus;
  modelVersion: string;
  reasoningHighlights: string[];
  heatmapMatrix?: number[][];     // Grid array for visual evidence heatmap overlay
...
export interface AnalysisCase {
  ...
  authenticityScore: number;     // 100 - confidence if manipulated, etc.
